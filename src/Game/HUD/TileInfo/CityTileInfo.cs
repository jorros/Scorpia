...
public class CityTileInfo : ITileInfo
{
    private readonly TileInfoNode _infoNode;
        ...
        {
            _infoNode.AddStat("population", $"{location.Population.Value.Format()} / {location.MaxPopulation.Value.Format()}",
                new TooltipDescription("Population",
                    $"The current population. Living space can be increased by building houses or upgrading the {location.Type.Value} to a higher tier", string.Empty, TooltipPosition.Info));
            ...
            _infoNode.AddStat("food",
                ...
                new TooltipDescription("Food",
                    $"You are currently storing {location.FoodStorage.Value.Format()} and the monthly food production is at {location.FoodProduction.Value.Total.FormatBalance()}. You can increase the storage room by upgrading this {location.Type.Value}. All produced food that cannot stored here will be transferred to your global storage.", string.Empty, TooltipPosition.Info));

            _infoNode.AddStat("garrison", location.Garrison.Value.Format(),
                new TooltipDescription("Garrison",
                    $"There are currently {location.Garrison.Value.Format()} troops stationed in this {location.Type.Value}", string.Empty, TooltipPosition.Info));
        }
}