...
public class NewLineRenderer : IFontRenderer
{
    ...
    public IEnumerable<(IntPtr texture, SDL.SDL_Rect target)> Render(Font font, IBlock block, Point position, ref Point cursor)
    {
        cursor.X = position.X;
        cursor.Y += ((NewLineBlock)block).LineHeight;

        return Enumerable.Empty<(IntPtr texture, SDL.SDL_Rect target)>();
    }
